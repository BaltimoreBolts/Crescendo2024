...
public class Swerve extends SubsystemBase {
  ...
  public Swerve() {
    ...
    for (int i = 0; i < 4; i++) {
      m_moduleRawVelocityEntries.add(
          new DoubleLogEntry(log, "/swerve/moduleRawVelocity[" + i + "]"));
  ...
  /**
   * This is called a command factory method, and these methods help reduce the number of files in
   * the command folder, increasing readability and reducing boilerplate.
   *
   * <p>Double suppliers are just any function that returns a double.
   */
  public Command drive(
    ...
    return new RunCommand(
            () -> {
              ...
              // Adding deadzone.
              forwardBack =
                  Math.abs(forwardBack) < Constants.kControls.AXIS_DEADZONE ? 0 : forwardBack;
  ...
  private void setModuleStates(SwerveModuleState[] states, boolean isOpenLoop) {
    ...
    SwerveDriveKinematics.desaturateWheelSpeeds(
        states, Constants.kSwerve.MAX_VELOCITY_METERS_PER_SECOND);
  ...
  public void periodic() {
    ...
    for (SwerveModule module : modules) {
      SmartDashboard.putNumber(
          String.format("Thrifty angle %d", module.moduleNumber),
          module.getThriftyAngle().getDegrees());
      SmartDashboard.putNumber(
          String.format("Max angle %d", module.moduleNumber),
          module.getSteerAngle().getDegrees());
      SmartDashboard.putNumber(
          String.format("Distance %d", module.moduleNumber), module.getDisance());
      ...
      SmartDashboard.putNumber(
          String.format("percentPower %d", module.moduleNumber), module.getDriveOutputPower());
      SmartDashboard.putNumber(
          String.format("rawVelocity %d", module.moduleNumber), module.getDriveRawVelocity());
      SmartDashboard.putNumber(
          String.format("driveError %d", module.moduleNumber), Math.abs(module.getDriveError()));

      SmartDashboard.putNumber(
          String.format("inchVelocity %d", module.moduleNumber),
          Units.metersToInches(module.getDriveVelocity()));
      SmartDashboard.putNumber(
          String.format("inchError %d", module.moduleNumber),
          Units.metersToInches(Math.abs(module.getVeolcityError())));
    }
